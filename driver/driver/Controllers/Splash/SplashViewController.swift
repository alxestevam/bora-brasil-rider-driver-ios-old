    ...
    func connectionError(error: ConnectionError) {
        ...
        case .NotFound:
            let title = NSLocalizedString("Message", comment: "Message Default Title")
             let message = NSLocalizedString("Alert_User_Info_Not_Found", comment: "")
            let titleAction = NSLocalizedString("Register", comment: "")
            let cancelAction = NSLocalizedString("Cancel", comment: "")
            let dialog = UIAlertController(title: title, message: message, preferredStyle: .alert)
            dialog.addAction(UIAlertAction(title: titleAction, style: .default) { action in
                self.onLoginClicked(self.buttonLogin)
            })
            dialog.addAction(UIAlertAction(title: cancelAction, style: .cancel, handler: nil))
