...
class DriverTravelViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    func showConfirmationDialog() {
        ...
        let message = NSLocalizedString("Alert_Finish_Service", comment: "")
        ...
        Finish(confirmationCode: finishService.confirmationCode, distance: finishService.distance ?? 0, log: finishService.log ?? "").execute() { result in
            ...
            case .success(let response):
                ...
                if response.status {
                    ...
                    let message = NSLocalizedString("Payment_Successful", comment: "")
    ...
    private func refreshScreen(travel: Request = Request.shared) {
        ...
        case .RiderCanceled, .DriverCanceled:
            ...
            let message = NSLocalizedString("Services_Canceled", comment: "")
            let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
        ...
        case .Finished, .WaitingForReview:
            DialogBuilder.alertOnSuccess(message: NSLocalizedString("Paid", comment: ""))
            self.navigationController?.popViewController(animated: true)
            
        default:
            let title = NSLocalizedString("Error", comment: "")
            let message = NSLocalizedString("Unkown_Status", comment: "")
    ...
    func openNavigationMenu(location: CLLocationCoordinate2D) {
        let title = NSLocalizedString("Navigation", comment: "")
        let message = NSLocalizedString("Select_App_Navigation", comment: "")
