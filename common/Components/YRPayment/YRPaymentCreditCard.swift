...
public final class YRPaymentCreditCard: UIView, CAAnimationDelegate {
    ...
    var cardNumber: String! {
        didSet {
            if (editing) {
                ...
                if cardNumber == "" {
                    let xes = NSMutableAttributedString(string: "9999 9999 9999 9999", attributes: [
        ...
        didSet {
            if (editing) {
                ...
                if cardHolderName == "" {
                    ...
                    let placeholder = NSMutableAttributedString(string: "NOME", attributes: [
                        NSAttributedString.Key.foregroundColor: UIColor.gray,
                    ])
                ...
                unselectAll()
                cardHolderNameLabel.text = cardHolderName.uppercased()
    ...
    var cardValidity: String! {
        didSet {
            if (editing) {
                ...
                unselectAll()
                cardValidityLabel.text = cardValidity
                cardValidityLabel.textColor = .white
            }
        }
    }
    ...
    static var registerFonts: Void = {
        let fontUrl = Bundle.main.url(forResource: "OCRAStd", withExtension: "otf")
    ...
    let cardHolderNameLabel: UILabel = {
        ...
        let placeholder = NSMutableAttributedString(string: "USUÁRIO BORA BRASIL", attributes: [
            NSAttributedString.Key.foregroundColor: UIColor.gray,
        ])
        name.append(placeholder)
    ...
    let cardValidityLabel: UILabel = {
        ...
        let placeholder = NSMutableAttributedString(string: "MM/AA", attributes: [
            NSAttributedString.Key.foregroundColor: UIColor.gray,
        ])
        nums.append(placeholder)
        lab.attributedText = nums
    ...
    let cardCryptogramLabel: UILabel = {
        ...
        let lab = UILabel()
        lab.translatesAutoresizingMaskIntoConstraints = false
        lab.font = UIFont(name: "OCRAStd", size: 12)
        let nums = NSMutableAttributedString(string: "", attributes: [
            NSAttributedString.Key.foregroundColor: UIColor.white,
        ])
        let xes = NSMutableAttributedString(string: "XXX", attributes: [
    ...
    func setupViews() {
        backgroundColor = Color.gray.rgb_29_28_30
    ...
    fileprivate func setupFaceView() {
        ...
        cardNumberLabel.leftAnchor.constraint(equalTo: faceView.leftAnchor, constant: 16).isActive = true
        cardNumberLabel.centerXAnchor.constraint(equalTo: faceView.centerXAnchor).isActive = true
        cardNumberLabel.widthAnchor.constraint(equalToConstant: 180).isActive = true
        ...
        cardValidityLabel.widthAnchor.constraint(equalToConstant: 60).isActive = true
        ...
        let cardHolderNrStaticLabel = UILabel()
        cardHolderNrStaticLabel.font = UIFont(name: "AppleSDGothicNeo-Medium", size: 10)
        cardHolderNrStaticLabel.translatesAutoresizingMaskIntoConstraints = false
        cardHolderNrStaticLabel.text = "NÚMERO CARTÃO"
        cardHolderNrStaticLabel.textColor = .white
        faceView.addSubview(cardHolderNrStaticLabel)
        cardHolderNrStaticLabel.widthAnchor.constraint(equalTo: cardNumberLabel.widthAnchor).isActive = true
        cardHolderNrStaticLabel.heightAnchor.constraint(equalToConstant: 12).isActive = true
        cardHolderNrStaticLabel.bottomAnchor.constraint(equalTo: cardNumberLabel.topAnchor, constant: -4).isActive = true
        cardHolderNrStaticLabel.leftAnchor.constraint(equalTo: leftAnchor, constant: 16).isActive = true
        ...
        cardHolderNameStaticLabel.text = "NOME"
        ...
        cardValidityStaticLabel.text = "VÁLIDO ATÉ"
    ...
    public init(type: YRCreditCardType, isEditing: Bool) {
